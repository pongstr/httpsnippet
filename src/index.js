...
// note: debug utilizes `window` object, but would like to run this lib inside webworker.
const debug = console.info; // require("debug")("httpsnippet");
// note: utlilize browser FormData instead
const MultiPartForm = FormData;
...
HTTPSnippet.prototype.prepare = function (request) {
  ...
  if (request.cookies && request.cookies.length) {
    request.cookiesObj = request.cookies.reduceRight(function (
      cookies,
      cookie
    ) {
    ...
    case "multipart/alternative":
      ...
      if (request.postData.params) {
        ...
        ***************/
        ...
        // override.
        const contentTypeHeader = helpers.hasHeader(
          request.headersObj,
          "content-type"
        )
...
// exports
module.exports.HTTPSnippet = HTTPSnippet;

module.exports.addTarget = function (target) {
  if (!("info" in target)) {
    throw new Error(
      "The supplied custom target must contain an `info` object."
    );
  } else if (
    !("key" in target.info) ||
...
module.exports.addTargetClient = function (target, client) {
  ...
  if (!targets.hasOwnProperty(target)) {
    ...
    throw new Error(
      "The supplied custom target client must contain an `info` object."
    );
    ...
    throw new Error(
      "The supplied custom target client already exists, please use a different key"
    );
  }
